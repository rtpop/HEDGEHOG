...
comm_scores_log <- log2(comm_scores + 1)
path_scores_log <- log2(path_scores + 1)
...
comm_heat <- pheatmap(comm_scores_bin, cluster_rows = TRUE, cluster_cols = comm_hclust, labels_row = NULL, labels_col = NULL, color = bw_palette)
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))
ggsave(comm_heat, file="ELAND/results/comm_heatmap_hclust_binomial_binary.pdf")

pway_heat <- pheatmap(path_scores_bin, cluster_rows = TRUE, cluster_cols = path_hclust, labels_row = NULL, labels_col = NULL, color = bw_palette)
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))
ggsave(pway_heat, file="ELAND/results/path_heatmap_hclust_binomial_binary.pdf")
...
comm_dist <- dist.cosine(t(as.matrix(comm_scores[,-1])))
path_dist <- dist.cosine(t(as.matrix(path_scores[,-1])))
...
comm_scores_bin <- ifelse(comm_scores > 0, 1, 0)
path_scores_bin <- ifelse(path_scores > 0, 1, 0)
rownames(comm_scores_bin) <- NULL
rownames(path_scores_bin) <- NULL
colnames(comm_scores_bin) <- NULL
colnames(path_scores_bin) <- NULL
bw_palette <- colorRampPalette(c("white", "black"))(2)
...
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))
ggsave(comm_heat, file="ELAND/results/comm_heatmap_hclust_cosine_binary.pdf") 

pway_heat <- pheatmap(path_scores_bin, cluster_rows = TRUE, cluster_cols = path_hclust, labels_row = NULL, labels_col = NULL, color = bw_palette)
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))
ggsave(pway_heat, file="ELAND/results/path_heatmap_hclust_cosine_binary.pdf")
...
library(netZooR)
# load mut data
load("/storage/kuijjerarea/tatiana/mutation_data/mut.ucec.RData")

mut <- as.matrix(mut.ucec)
...
#run sambar
#sambar_res <- sambar(mutdata = mut, esize = esize, cangenes = genes, signatureset = "ELAND/data/c2.cp.v2024.1.Hs.symbols.gmt")

# convert gmt
edg <- netZooR:::sambarConvertgmt("ELAND/data/c2.cp.v2024.1.Hs.symbols.gmt", cagenes = genes)
...
save(signpat, file = "ELAND/results/sambar_path_scores_ucec.RData")
```
...
save(signpat, file = "ELAND/results/sambar_comm_scores_ucec_w_breast_comm.RData")
```

```{r plot_heatmap_ucec}
load("ELAND/results/sambar_comm_scores_ucec_w_breast_comm.RData")
...
# calculate binomial distance between samples
  ...
  cluster <- dendextend::rotate(cluster, order_Rt = TRUE, decreasing = FALSE)
  cluster <- as.hclust(cluster)

bw_palette <- colorRampPalette(c("white", "black"))(10)

#plot heatmaps
comm_bin <- pheatmap(comm_scores_log, cluster_rows = FALSE, cluster_cols = cluster, labels_row = NULL, labels_col = NULL, color = bw_palette)
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))
ggsave(comm_bin, file="ELAND/results/comm_heatmap_log10_ucec_w_breast_comm.pdf")
```
...
# Assuming comm_scores_log is your data matrix
# Define a black-to-white color palette
bw_palette <- colorRampPalette(c("white", "black"))(400)

# Set the scale cap and breaks
scale_cap <- 0.04
breaks <- seq(0, scale_cap, length.out = length(bw_palette) + 1)
breaks[length(breaks)] <- Inf # Ensure values >= 0.05 are capped

# Create the heatmap with specified color breaks
comm_bin <- pheatmap(
  comm_scores_log,
  cluster_rows = FALSE,
...
# Optionally customize the legend text using grid functions
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))

# Hack to modify legend (if more customization is needed manually)
# (comm_bin$gtable would be the object containing legend, manipulate if needed)
...
# sample names
samples <- comm_scores$V1

# run pca
pca_brca <- pcaMethods::pca(t(as.matrix(comm_scores[,-1])), method = "svd", center = TRUE)

# plot pca scores
scores <- pca_brca@scores
p <- ggplot(scores, aes(x=PC1, y=PC2)) +
  geom_point() +
  labs(title="PCA of community scores", x="PC1", y="PC2") +
  theme_classic()

ggsave(p, file="ELAND/results/pca_comm_scores_brca.pdf")

```

```{r sparse_svd_with_irlba}
# doesn't really work, but I'll leave it here for now
library(irlba)
library(cluster)
...
# run sparse svd
sparse_svd <- irlba(comm_scores, nu = 0, nv=10)

# reduce so you can then cluster
comm_scores_reduced <- as.matrix(comm_scores %*% sparse_svd$v)

# cluster
clust_kmeans <- kmeans(comm_scores, 10)
summary(silhouette(clust_kmeans$cluster, dist(comm_scores_reduced)))
```

```{r affinity_propagation_clustering}
library(apcluster)
comm_scores <- fread("ELAND/results/pt_out_comm.csv")
