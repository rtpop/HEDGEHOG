...
```{r plot_heatmap_ucec}
load("ELAND/results/sambar_path_scores_ucec.RData")
...
#plot heatmaps
comm_bin <- pheatmap(comm_scores_bin, cluster_rows = FALSE, cluster_cols = cluster, labels_row = NULL, labels_col = NULL, color = bw_palette)
grid.text("samples", x = unit(0.5, "npc"), y = unit(-0.03, "npc"), gp = gpar(fontsize = 12))
grid.text("communities", x = unit(-0.03, "npc"), y = unit(0.5, "npc"), rot = 90, gp = gpar(fontsize = 12))
ggsave(comm_bin, file="ELAND/results/path_heatmap_binary_ucec.pdf")
```
```{r pca_comm_scores}
# load scores
comm_scores <- fread("ELAND/results/pt_out_comm.csv")
